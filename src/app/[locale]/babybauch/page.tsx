...
export async function generateMetadata({
  ...
  const page = getPage(locale, "babybauch");

  return {
    title: page?.frontmatter.metaTitle || page?.frontmatter.title || "Babybauchshooting",
    description: page?.frontmatter.metaDescription || page?.frontmatter.description,
  };
}

export default async function BabybauchPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
